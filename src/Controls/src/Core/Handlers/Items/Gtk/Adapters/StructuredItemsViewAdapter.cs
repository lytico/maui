...
namespace Microsoft.Maui.Controls.Handlers.Items
{
		...
		where TItemsViewSource : IItemsViewSource
	{
		...
		protected internal StructuredItemsViewAdapter(TItemsView itemsView,
			Func<View, ItemContentView> createItemContentView = null) : base(itemsView, createItemContentView)
		{
			UpdateHasHeader();
			UpdateHasFooter();
		}
		...
		public override RecyclerView.ViewHolder OnCreateViewHolder(Gtk.Widget parent, int viewType)
		{
			...
			{
				return CreateHeaderFooterViewHolder(ItemsView.Header, ItemsView.HeaderTemplate);
			}

			if (viewType == ItemViewType.Footer)
			{
				return CreateHeaderFooterViewHolder(ItemsView.Footer, ItemsView.FooterTemplate);
			}
		...
		protected RecyclerView.ViewHolder CreateHeaderFooterViewHolder(object content, DataTemplate template)
		{
			if (template != null)
			{
				var footerContentView = new ItemContentView();
			...
			return SimpleViewHolder.FromText(content?.ToString(), fill: false);
		}
