...
namespace Microsoft.Maui.Controls.Handlers.Items
{
	...
	public class EmptyViewAdapter : RecyclerView.Adapter
	{
		...
		public override RecyclerView.ViewHolder OnCreateViewHolder(Gtk.Widget parent, int viewType)
		{
			...
			{
				return CreateHeaderFooterViewHolder(Header, HeaderTemplate);
			}

			if (viewType == _footerViewType)
			{
				return CreateHeaderFooterViewHolder(Footer, FooterTemplate);
			}
		...
		protected RecyclerView.ViewHolder CreateHeaderFooterViewHolder(object content, DataTemplate template)
		{
			if (template != null)
			{
			...
			return SimpleViewHolder.FromText(content?.ToString(), false);
		}

		protected RecyclerView.ViewHolder CreateEmptyViewHolder(object content, DataTemplate template, Gtk.Widget parent)
		{
			if (template == null)
			{
				if (content is not View formsView)
				{
					...
					return SimpleViewHolder.FromText(content?.ToString());
				}

				// EmptyView is a Forms View; display that
				return SimpleViewHolder.FromFormsView(formsView, () => GetWidth(parent), () => GetHeight(parent), ItemsView);
			}

			var itemContentView = new SizedItemContentView(() => GetWidth(parent), () => GetHeight(parent));
		...
		int GetHeight(Gtk.Widget parent)
		{
		...
		int GetWidth(Gtk.Widget parent)
		{
