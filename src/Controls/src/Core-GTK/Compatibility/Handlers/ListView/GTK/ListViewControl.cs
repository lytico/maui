...
namespace Microsoft.Maui.Controls.Handlers.Compatibility
{
	...
	public class ItemTappedEventArgs : EventArgs
	{
		...
		private GtkMouseButton _gtkMouseButton;
		...
		public GtkMouseButton GtkMouseButton
		{
			get
			{
				return _gtkMouseButton;
			}
		}

		public ItemTappedEventArgs(object item, GtkMouseButton button)
		{
			_item = item;
			_gtkMouseButton = button;
		}
	}
	...
	public class ListViewSeparator : EventBox
	{
		public ListViewSeparator()
		{
			...
			VisibleWindow = false;
			ButtonPressEvent += ListViewSeparator_ButtonPressEvent;
		}

		private void ListViewSeparator_ButtonPressEvent(object o, ButtonPressEventArgs args)
		{
	...
	public class ListViewControl : ScrolledWindow
	{
		...
		private MauiGTKButton _refreshButton;
		...
		public static Gdk.Color DefaultSelectionColor = new Gdk.Color(0x34, 0x98, 0xD8); //Color.FromHex("#3498DB").ToGtkColor();
		...
		private void BuildListView()
		{
			...
			// Refresh Button
			_refreshButton = new MauiGTKButton("Refresh", string.Empty, Stock.Refresh);
			//_refreshButton.GetInternalLabel.Markup = "<span>Refresh</span>";
			//_refreshButton.LabelWidget.SetTextFromSpan(
			//	new Span()
			//	{
			//		Text = "Refresh"
			//	});
			//_refreshButton.ImageSetStock = Stock.Refresh;
		...
		private void LazyLoadItems(List items)
		{
			...
			_data.LoadState = Handlers.Compatibility.State.Started;
		...
		private bool LoadItems()
		{
			...
			var isLoading = (id.LoadState == Handlers.Compatibility.State.Started) ||
				(id.LoadState == Handlers.Compatibility.State.Loading);

			if (!isLoading)
			{
				id.LoadState = Handlers.Compatibility.State.Completed;
				return false;
			}
		...
		private void UpdateItem(CellBase cell)
		{
			cell.ButtonPressEvent += (sender, args) =>
			{
				...
				if (gtkCell != null && gtkCell.Cell != null)
				{
					...
					OnItemTapped?.Invoke(this, new ItemTappedEventArgs(SelectedItem, (GtkMouseButton)args.Event.Button - 1));
				}
			};
		...
		{
			Debug.Assert(_data.LoadState == Handlers.Compatibility.State.Completed);
