...
";
		...
		public static void GenerateXamlCodeBehind(string fileSource, string fileNameRoot, string absolutePathName, string relativePathName, string projectOutputDirectory)
		{
			...
			var outputFileWithExt = fileNameRoot + ".i.g.cs";
			var outputFileName = Path.Combine(projectOutputDirectory, outputFileWithExt);
			...
			if (!TryParseXaml(fileSource, out var rootType, out var rootClrNamespace, out var generateDefaultCtor, out var addXamlCompilationAttribute, out var hideFromIntellisense, out var XamlResourceIdOnly, out var baseType, out var namedFields, out var parseException))
			{
				if (parseException != null)
					Console.WriteLine("ERROR: TryParseXaml failed.");
			...
			//if (projItem.ManifestResourceName != null && projItem.TargetPath != null)
				sb.AppendLine($"[assembly: global::Microsoft.Maui.Controls.Xaml.XamlResourceId(\"{fileNameRoot}\", \"{absolutePathName.Replace('\\', '/')}\", {(rootType == null ? "null" : "typeof(global::" + rootClrNamespace + "." + rootType + ")")})]");

			if (XamlResourceIdOnly)
			{
				// Write source file to project directory
			...
			sb.AppendLine("{");
			sb.AppendLine($"\t[global::Microsoft.Maui.Controls.Xaml.XamlFilePath(\"{relativePathName?.Replace("\\", "\\\\")}\")]");
			...
			Console.WriteLine($"Generated file for: {fileNameRoot}");
		}
		...
		static bool TryParseXaml(string fileSource, out string? rootType, out string? rootClrNamespace, out bool generateDefaultCtor, out bool addXamlCompilationAttribute, out bool hideFromIntellisense, out bool xamlResourceIdOnly, out string? baseType, out IEnumerable<(string, string, string)>? namedFields, out Exception? exception)
		{
			...
			namedFields = GetNamedFields(root, nsmgr);
			var typeArguments = GetAttributeValue(root, "TypeArguments", XamlParser.X2006Uri, XamlParser.X2009Uri);
			baseType = GetTypeName(new XmlType(root.NamespaceURI, root.LocalName, typeArguments != null ? TypeArgumentsParser.ParseExpression(typeArguments, nsmgr, null) : null));
		...
		static IEnumerable<(string name, string type, string accessModifier)> GetNamedFields(XmlNode root, XmlNamespaceManager nsmgr)
		{
			...
			foreach (XmlNode node in names)
			{
				...
				yield return (name ?? "", GetTypeName(xmlType), accessModifier);
			}
		}

		static string GetTypeName(XmlType xmlType)
		{
			string returnType;
			...
			if (ns != null)
			{
				...
				if (xmlType.TypeArguments != null)
					returnType = $"{returnType}<{string.Join(", ", xmlType.TypeArguments.Select(typeArg => GetTypeName(typeArg)))}>";
